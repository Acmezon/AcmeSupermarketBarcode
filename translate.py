...
def checksum(barcode):
    ...
    term1 = int(barcode[0]) + int(barcode[2]) + int(barcode[4]) + int(barcode[6]) + int(barcode[8]) + int(barcode[10])
    # 2. Multiply this result by 3.
    term2 = 3 * term1
    # 3. Add the values of the digits in positions 2, 4, 6, 8, and 10.
    term3 = int(barcode[1]) + int(barcode[3]) + int(barcode[5]) + int(barcode[7]) + int(barcode[9])
    ...
    if len(vector) == 95 and \
        ...
        for group in range(0, 6):
            ...
            if left[0] == 0 and \
                ...
                print("Error: Group " + str(group) + " wrong format. Parity not matching.")
                return -1
        ...
        print("Error: Wrong format. Incorrent barcode length or frontier bars not matching.")
        return -1
